...
export const CRMProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    if (!isFirebaseEnabled || !db) {
      ...
      setReminders(loadState("reminders", []))
    ...
    const unsubscribes = [
      ...
      onSnapshot(collection(db, "reminders"), (snap) => {
        ...
        if (list.length > 0) {
          ...
          setReminders([])
          localStorage.setItem("servicebuddy_reminders", JSON.stringify([]))
        }
      }, (err) => handleListenerError(err, "reminders"))
    ]
  ...
  useEffect(() => {
    if (reminders.length) localStorage.setItem("servicebuddy_reminders", JSON.stringify(reminders))
  }, [reminders])
  useEffect(() => {
  ...
  const addReminder = (rem: Omit<Reminder, "id">) => {
    const remNum = reminders.length ? Math.max(...reminders.map(r => {
    ...
    if (isFirebaseEnabled && db) {
      setDoc(doc(db, "reminders", newRem.id), newRem)
    } else {
